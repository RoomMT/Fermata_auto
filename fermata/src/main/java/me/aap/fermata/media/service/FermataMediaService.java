...
public class FermataMediaService extends MediaBrowserServiceCompat {
    ...
    private static final String MEDIA_SESSION_HAS_VIDEO = "com.google.android.gms.car.media.MEDIA_SESSION_HAS_VIDEO";
    private static final String VIDEO_FOCUS_REQUEST = "com.google.android.projection.gearhead.VIDEO_FOCUS_REQUEST";
    ...
    private Action actionPrev, actionRw, actionPlay, actionPause, actionFf, actionNext, actionFavAdd, actionFavRm;
    private Bitmap defaultAudioIcon, defaultVideoIcon;
    private PowerManager.WakeLock wakeLock;
    private WifiManager.WifiLock wifiLock;

    // [FIX] Handler để trì hoãn việc nhả khóa (Keep-Alive)
    private final Handler delayedStopHandler = new Handler(Looper.getMainLooper());
    private final Runnable releaseLocksRunnable = this::releaseLocksNow;
    ...
    public void onCreate() {
        ...
        wakeLock = pm.newWakeLock(PowerManager.PARTIAL_WAKE_LOCK, "Fermata:MediaService");
        wakeLock.setReferenceCounted(false);

        WifiManager wm = (WifiManager) getApplicationContext().getSystemService(Context.WIFI_SERVICE);
        if (wm != null) {
            wifiLock = wm.createWifiLock(WifiManager.WIFI_MODE_FULL_HIGH_PERF, "Fermata:WifiLock");
    ...
    public void onDestroy() {
        ...
        releaseLocksNow(); // Đảm bảo nhả khóa khi destroy
        super.onDestroy();
    ...
    // ... [Các phương thức onStartCommand, onBind, onGetRoot... giữ nguyên] ...
    @Override
    ...
    public BrowserRoot onGetRoot(@NonNull String clientPackageName, int clientUid, Bundle rootHints) {
        ...
        // Thêm cờ hỗ trợ Video cho AA
        extras.putBoolean(MEDIA_SESSION_HAS_VIDEO, true);
        extras.putInt(VIDEO_FOCUS_REQUEST, 1);
    ...
    void updateNotification(int st, PlayableItem currentItem) {
        // [FIX] Hủy bỏ lệnh nhả khóa đang chờ (nếu có)
        delayedStopHandler.removeCallbacks(releaseLocksRunnable);
        ...
        switch (st) {
            ...
            case STATE_PAUSED -> {
                ...
                // [FIX] KHÔNG nhả khóa ngay lập tức. Giữ thêm 2 phút.
                // Việc này giữ cho socket kết nối của Youtube không bị đóng ngay khi bạn Pause
                // hoặc tắt màn hình.
                // Giúp resume mượt mà, đặc biệt là video độ phân giải thấp.
                delayedStopHandler.postDelayed(releaseLocksRunnable, 90000);
            }
            case STATE_PLAYING -> {
                startForeground(NOTIF_ID, createNotification(st, currentItem));

                // Giữ khóa ngay lập tức
    ...
    // ... [Phần forceVideoFocus, createNotification và notificationInit giữ nguyên
    // như cũ] ...

    private Notification createNotification(int st, PlayableItem i) {
        if (notifContentIntent == null)
            notificationInit();
        ...
        if (mediaMetadata != null) {
            ...
            if (callback.isDefaultImage(largeIcon)) {
                if (i != null && i.isVideo()) {
                    if (defaultVideoIcon == null)
                        defaultVideoIcon = createLargeIcon(R.drawable.video);
                    largeIcon = defaultVideoIcon;
                } else {
                    if (defaultAudioIcon == null)
                        defaultAudioIcon = createLargeIcon(R.drawable.audiotrack);
    ...
    public void notificationInit() {
        ...
        actionFavAdd = new Action(R.drawable.favorite, getString(R.string.favorites_add), pi(INTENT_FAVORITE_ADD));
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            NotificationChannel nc = new NotificationChannel(NOTIF_CHANNEL_ID, getString(R.string.media_service_name),
        ...
        ContextCompat.registerReceiver(this, intentReceiver, filter, ContextCompat.RECEIVER_NOT_EXPORTED);
    }
    ...
    public void forceVideoFocus() {
        ...
        // 2. Update Extras to hint AA about Video Focus
        Bundle extras = session.getController().getExtras();
        if (extras == null)
            extras = new Bundle();
        extras.putInt("me.aap.fermata.video_focus_request", 1);
        session.setExtras(extras);

        // 3. Refresh PlaybackState to ensure AA picks up the change
    ...
    public Handler getHandler() {
        return FermataApplication.get().getHandler();
    }
