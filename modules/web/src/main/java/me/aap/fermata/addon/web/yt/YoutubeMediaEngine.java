 ...
 *         Optimized for Service Integration & Background Playback
 */
class YoutubeMediaEngine implements MediaEngine, OverlayMenu.SelectionHandler {
    ...
    public YoutubeMediaEngine(YoutubeWebView web, MainActivityDelegate a) {
        this.web = web;
        this.cb = a.getMediaSessionCallback();
    ...
    void playing(String url) {
        ...
        if (cb != null) {
            ...
            cb.forceVideoFocus(); // Trigger AA Focus explicitly
        }
        cb.onPlay();
    }
    ...
    void paused() {
        ...
        // We set ignorePause = true when we request focus.
        // If WebView pauses (due to focus loss or otherwise) during this window,
        // we Force Resume it to keep the audio flowing.
        if (ignorePause) {
            if (web != null)
                web.play();
            return;
    ...
    @Override
    public void stop() {
        // [FIX] Always stop webview if service requests stop
    ...
    public FutureSupplier<Long> getDuration() {
        return (web != null) ? web.getDuration() : completed(0L);
    }

    @Override
    public FutureSupplier<Long> getPosition() {
        return (web != null) ? web.getPosition() : completed(0L);
    }
    ...
    public FutureSupplier<Float> getSpeed() {
        return (web != null) ? web.getSpeed() : completed(1f);
    }
    ...
    // ... (Phần còn lại giữ nguyên y hệt bản gốc) ...
    @Override
    ...
    public boolean requestAudioFocus(@Nullable AudioManager audioManager,
        ...
        // Real Audio Focus Request for Android Auto
        // We MUST request system focus for Android Auto to recognize us.
        // This process might cause a transient focus loss in the WebView, which we
        // ignore via MediaSessionCallback.

        ignorePause = true;

        boolean granted = false;
        if (audioManager != null && audioFocusReq != null) {
            granted = (androidx.media.AudioManagerCompat.requestAudioFocus(audioManager,
                    audioFocusReq) == android.media.AudioManager.AUDIOFOCUS_REQUEST_GRANTED);
        }

        // Reset ignorePause after a safety delay to allow the focus state to settle.
        if (web != null) {
            web.postDelayed(() -> ignorePause = false, 2000);
        } else {
            ignorePause = false;
        }
    ...
    public void releaseAudioFocus(@Nullable AudioManager audioManager,
            @Nullable AudioFocusRequestCompat audioFocusReq) {
        if (audioManager != null && audioFocusReq != null) {
            androidx.media.AudioManagerCompat.abandonAudioFocusRequest(audioManager, audioFocusReq);
        }
    }
    ...
    private final class Current extends YoutubeItem {
        ...
        public Current(String url) {
            super(url, mediaRoot, GenericFileSystem.getInstance().create(url));
        }
